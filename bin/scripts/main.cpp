//compilar con: g++ -Wall -lpthread -o main main.cpp songs.cpp player.cpp
...
string fileSongs[100]; //Canciones en la carpeta CHANNEL_0
float timeSongs[100]; //Tiempos
unsigned size_fileSongs;

void lectura();
void lectura_songs();
void creacion(string c);
void run_th(bool& fin, Music_Player &player, bool& menu, string& cancion_reproduciendo);
void menu_th(bool& fin, Music_Player &player, bool& menu, bool& estoy_menu);
void reproduciendo_th(bool& fin, bool& estoy_menu, bool& menu, string& cancion_reproduciendo);
...
int main(){
  ...
  thread th_run(run_th, ref(fin), ref(player), ref(menu), ref(cancion_reproduciendo));
	thread th_reproduciendo(reproduciendo_th, ref(fin), ref(estoy_menu), ref(menu), ref(cancion_reproduciendo));
		...
		estoy_menu = false;
			...
			switch(opcion[0]){
				...
				case '1':
					...
					cout << "\033[1;41m____" << dias[unsigned(day[0] - '0') - 1] << " __ " << hour << " : " << min << " __ CANCION____CANAL__\033[1;0m" << endl;
					cout << endl;
					cout << "\033[1;37mCANCION? Ej:00, 01, 12...\033[1;37m" << endl;
					cout << endl;
					...
					while(!isdigit(mas[0]) || !isdigit(mas[1]) || (mas.length() != 2) || (stoi(mas) < 0) || (unsigned(stoi(mas)) > size_fileSongs)){
							cout << "\033[1;36m\\> "; cin >> mas; cout << "\033[1;0m";
					}
				...
				case '6':{
					...
					while(!isdigit(c1[0]) || !isdigit(c1[1]) || (c1.length() != 2)){
							cout << "\033[1;36mIzquierda -> "; cin >> c1; cout << "\033[1;0m";
						}
					if(stoi(c1) >= 0 && unsigned(stoi(c1)) < size_fileSongs){
						cout << "\033[1;36mDerecha -> "; cin >> c2; cout << "\033[1;0m";
						while(!isdigit(c2[0])  || !isdigit(c2[1]) || (c2.length() != 2)){
							cout << "\033[1;36mDerecha -> "; cin >> c2; cout << "\033[1;0m";
						}
						cout << "\033[1;36m";
